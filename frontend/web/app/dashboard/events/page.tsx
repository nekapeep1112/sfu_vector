...
export default function EventsPage() {
  ...
  const institutes = useMemo(() => {
    const all = EVENTS.flatMap((e) => e.org.split(',').map((s) => s.trim()));
    return Array.from(new Set(all)).sort((a, b) => a.localeCompare(b, 'ru'));
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (instituteFilter !== 'all') list = list.filter((e) => e.org.split(',').map((s) => s.trim()).includes(instituteFilter));
    return list;
  }, [tabFiltered, search, typeFilter, formatFilter, instituteFilter]);
      ...
      <div>
        ...
        <div className="row gap-3" style={{ marginBottom: 24, flexWrap: 'wrap' }}>
          ...
          <DropdownButton
            ...
            defaultLabel="Все институты"
            options={[{ value: 'all', label: 'Все институты' }, ...institutes.map((i) => ({ value: i, label: i }))]}
